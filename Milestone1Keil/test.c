...
static portTASK_FUNCTION( SomeTask, pvParameters )
{
	...
	for(;;)
	{
		...
		if (vtI2CEnQ(devPtr,0x01,0x4F,sizeof(i2cCmdReadVals),i2cCmdReadVals,12) != pdTRUE) {
			VT_HANDLE_FATAL_ERROR(0);
		}

		//wait for message from I2C
		if (vtI2CDeQ(devPtr,12,&messageReceived[0],&rxLen,&status) != pdTRUE) {
			VT_HANDLE_FATAL_ERROR(0);
		}
