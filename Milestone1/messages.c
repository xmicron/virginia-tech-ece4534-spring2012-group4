...
// THIS IS OUR I2C MESSAGE QUEUE ----------------------------------------------------
static msg_queue toI2C_MQ;
signed char	toI2C_sendmsg(unsigned char length,unsigned char msgtype,void *data)
{
/*#ifdef DEBUG
	if (!in_low_int()) {
...
signed char	toI2C_recvmsg(unsigned char maxlength,unsigned char *msgtype,void *data)
{
/*#ifdef DEBUG
	if (! in_main()) {
...
#endif*/
	return(recv_msg(&toI2C_MQ,maxlength,msgtype,data));
}
...
int block_on_To_I2Cqueue()
{
	if (check_msg(&toI2C_MQ))
	{
		return 1;
	}
	else
