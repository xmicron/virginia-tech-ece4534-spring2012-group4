	...
	if (msg_to_send) {
		...
		if(ic_ptr->buffer[0] == 0xaa)	{						// Check to make sure master sent 0xaa
			...
			if(block_on_FromMainHigh())	{						// Defined in messages.c	
				error = FromMainHigh_recvmsg(3, &type, (void *) TX_MSG);	// receive ADC value from FromMainHighMQ
				start_i2c_slave_reply(3,TX_MSG);							// send ADC value to Master via I2C
				// CHECK ERROR
				...
				start_i2c_slave_reply(3,TX_MSG);
			}
		}
