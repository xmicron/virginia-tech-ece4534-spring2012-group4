...
static portTASK_FUNCTION( I2CTask, pvParameters )
{
	...
	for(;;)
	{
		...
		if (vtI2CEnQ(devPtr,0x01,0x40,1,&ReturnADCValue,2) != pdTRUE) {
			VT_HANDLE_FATAL_ERROR(0);
		}

		//wait for message from I2C
		if (vtI2CDeQ(devPtr,2,&ADCValueReceived[0],&rxLen,&status) != pdTRUE) {
			//VT_HANDLE_FATAL_ERROR(0);
		} 


		if (vtI2CEnQ(devPtr,0x01,0x4F,1,&Gimmesomething,2) != pdTRUE) {
			VT_HANDLE_FATAL_ERROR(0);
		}

		//wait for message from I2C
		if (vtI2CDeQ(devPtr,2,&SecondaryReceived[0],&rxLen,&status) != pdTRUE) {
			//VT_HANDLE_FATAL_ERROR(0);
		}  
